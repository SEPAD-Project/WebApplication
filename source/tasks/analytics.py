...
@celery.task
def start_process_compare_classes(school_email):
    # Generate accuracy data and save the chart
...
@celery.task
def start_process_compare_teachers(school_email):
    # Generate accuracy data and save the chart
...
@celery.task
def start_process_student_accuracy_week(name, family, id, class_id, school_email):
    # Generate accuracy data and save the chart
    data = calculate_student_weekly_accuracy(str(class_id), str(id))
...
@celery.task
def start_process_compare_student_accuracy_by_lesson(name, family, id, class_id, school_email):
    # Calculate per-lesson accuracy and generate chart
    data = calculate_student_accuracy_by_lesson(str(class_id), str(id))
