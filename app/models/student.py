...
class Student(db.Model):
    ...
    # Last name (family name) of the student (required)
    student_family = db.Column(db.String(100), nullable=False)
    # Unique national code of the student (required)
    student_national_code = db.Column(
        db.String(100), nullable=False, unique=True)
    # Password for the student (required)
    student_password = db.Column(db.String(100), nullable=False)
    # Class code to which the student belongs (required)
    class_code = db.Column(db.String(100), nullable=False)
    # School code to which the student belongs (required)
    ...
    def __init__(self, student_name, student_family, student_national_code, student_password, class_code, school_code):
        ...
        # Set the national code of the student
